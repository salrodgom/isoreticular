...
subroutine output_gulp()
  ...
  do i=1,n_atoms
   ...
   write(u,'(a10,1x,3(f14.7,1x))')newlabel,(xcrystal(j,i),j=1,3)
  end do
  write(u,'(a,1x,i3)')'species',3
  write(u,'(a4,1x,a4,1x,a10)') "Si  ", "core", "Si    core"
  write(u,'(a4,1x,a4,1x,a10)') "O   ", "core", "O_O2- core"
  write(u,'(a4,1x,a4,1x,a10)') "O   ", "shel", "O_O2- shel" 
  write(u,'(a)') "end"
...
subroutine write_distances()
 ...
 do i=1,n_atoms
  if(label(i)==label_for_distance(1,1))then
   do j=1,n_atoms 
    if(j/=i.and.label(j)==label_for_distance(2,1).and.DistanceMatrix(i,j)<=RDF_max)then
