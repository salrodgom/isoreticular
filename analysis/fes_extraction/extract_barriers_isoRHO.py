...
Output per file:
    ...
    barr [kJ/mol], barr [kBT/Si], barr [100 kBT/Si]
...
hdr = (f"{'G_k':4s} {'file':12s} {'P[GPa]':>7s} {'p/p_c':>6s} {'N_T':>6s} "
       ...
       f"{'DG*[kJ/mol]':>12s} {'DG*[kBT/Si]':>12s} {'DG*[100kBT/Si]':>15s}")
print(hdr)
print('-' * len(hdr))
...
for member, dirpath in DIRS.items():
    ...
    for fpath in fes_files:
        ...
        barr_intens = barr_si / 100               # in manuscript units of 100 kBT/Si
        print(f"{member:4s} {fname:12s} {p_gpa:7.3f} {p_gpa/pc:6.2f} {n_t:>6d} "
...
print("% Columns: G_k, N_T, p_c [GPa] (soft-mode), p_FES [GPa], DG* [100 kBT/Si], DG* [kBT/cell]")
for m in ['G_1', 'G_2', 'G_3', 'G_4', 'G_5']:
    ...
    if r is None:
        ...
        print(f"${m}$ & {r['n_t']} & ${r['pc']:.4f}$ & ${r['p_gpa']:.3f}$ "
              f"& ${r['barr_intens']:.4f}$ & ${per_cell:.0f}$ \\\\")
