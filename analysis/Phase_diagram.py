...
cv_col=3
bias_col=8
rescale_cv=3 #to have crystallyinity from 0 to 1
...
for i in range(nbins):
  ...
  for j in range(nbins):
    print(t[i,j], p[i,j]/from_bar, -np.log(np.sum( weights(1/(kB*t[i,j]),p[i,j]))), file=outfile)
         # -np.log(np.sum(weights(1/(kB*t[i,j]),p[i,j])[cv>0.5])/np.sum(weights(1/(kB*t[i,j]),p[i,j])[cv<0.001])),file=outfile)
  print('',file=outfile)
